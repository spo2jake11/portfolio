(function () {
	...
	function register() {
		...
		var head = document.getElementById("signin_h");

		if (head) head.innerText = "Register";
		if (btnChange) btnChange.value = "Register";
		if (btnReg) btnReg.innerText = "Cancel";
		if (formConnect)
			formConnect.action =
				typeof window.CRUD_REGISTER_URL !== "undefined"
					? window.CRUD_REGISTER_URL
					: "register";
	...
	function cancel() {
		...
		if (head) head.innerText = "Sign In";
		if (btnChange) btnChange.value = "Sign in";
		if (btnReg) btnReg.innerText = "Register";
		if (formConnect && window.CRUD_VALIDATE_URL)
			formConnect.action = window.CRUD_VALIDATE_URL;
